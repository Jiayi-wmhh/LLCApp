...
def mainPage(request):
    return render(request, "stock/mainPage.html")

def sentimentAnalysis(request):
    return render(request,"stock/sentimentAnalysis.html")

def stockInfo(request):
    return render(request,"stock/stockInfo.html")

def showInfoResult(request):
    ticker = request.POST.get('ticker')
    apikey = "&token=c8kjg9qad3ibbdm3takg"
    res = []
    choice = request.POST.get('choice')
    if choice == 'Company_Info':
        # get companyinfo url
        ...
        resp4 = requests.get(news_base_url).json()
        ...
        resp5 = requests.get(chart_base_url).json()
        resp5 = json.dumps(resp5)
        res = resp5
    
    '''
    
    # get recommendation
    recommendation_base_url = "https://finnhub.io/api/v1/stock/recommendation?symbol="
    recommendation_base_url += ticker
    recommendation_base_url += apikey
    resp3 = requests.get(recommendation_base_url).json()
    resp3 = json.dumps(resp3)
    res.append(resp3)
    '''
  
    return render(request,"stock/INFOresult.html",{ "Result":res }) 

def showSAResult(request):
    ticker = request.POST.get('ticker')
    choice = request.POST.get('choice')
    result_json = []
    if choice == 'Twitter_Search':
            
        # adjust the numebr of tweets needed here
        num_twitter = 100
    ...
    return render(request,"stock/SAresult.html",{ "Result":result_json })
