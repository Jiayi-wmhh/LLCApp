...
def index(request):
  global terminal 
  return render(request, 'index.html', terminal)

def oneToAll(request):
  global terminal 
  return render(request, 'oneToAll.html', terminal)
...
def second(request):
    ...
    global terminal 
    terminal = {**terminal,**context}
    lat = [35.86166, 37.09024, 56.130366,
           23.634501, 36.204824, 35.907757,
           22.396428, 23.69781, 55.378051,
           51.165691, 46.227638, 52.132633,
           50.503887, 41.87194, 40.463667,
           51.919438, 1.352083, 14.058324,
           15.870032, 4.210484, -0.789275,
           12.879721, 61.52401, -14.235004,
           -30.559482, 20.593684, 30.375321,
           38.963745, -25.274398, -40.900557,
           46.818188, 23.885942, 23.424076,
           -38.416097, 48.019573, 22.198745]
    lon = [104.195397, -95.712891, -106.346771,
           -102.552784, 138.252924, 127.766922,
           114.109497, 120.960515, -3.435973,
           10.451526, 2.213749, 5.291266,
           4.469936, 12.56738, -3.74922,
           19.145136, 103.819836, 108.277199,
           100.992541, 101.975766, 113.921327,
           121.774017, 105.318756, -51.92528,
           22.937506, 78.96288, 69.345116,
           35.243322, 133.775136, 174.885971,
           8.227512, 45.079162, 53.847818,
           -63.616672, 66.923684, 113.543873]
    text = ['China', 'USA', 'Canada',
            'Mexico', 'Japan', 'South Korea',
            'Hong Kong', 'Taiwan', 'United Kingdom',
            'Germany', 'France', 'Netherlands',
            'Belgium', 'Italy', 'Spain',
            'Poland', 'Singapore', 'Vietnam',
            'Thailand', 'Malaysia', 'Indonesia',
            'Philippines', 'Russia', 'Brazil',
            'South Africa', 'India', 'Pakistan',
            'Turkey', 'Australia', 'New Zealand',
            'Switzerland', 'Saudi Arabia', 'United Arab Emirates',
            'Argentina', 'Kazakhstan', 'Macau']
    ...
    fig.add_trace(
        go.Scattergeo(
            lon=[lon[text.index(context['fc'])],
                 lon[text.index(context['sc'])]],
            lat=[lat[text.index(context['fc'])],
                 lat[text.index(context['sc'])]],
            mode='lines+markers',
    ...
    plot(fig, validate=False, filename='./templates/fig.html',
         auto_open=False)
...
def showdata(request):
	global terminal
	return render(request, 'showdata.html', terminal)
...
def showalldata(request):
  ...
  context = {"country": country, "product": product, "Datestart": Datestart, "Dateend": Dateend}
  terminal = {**terminal,**context}
  ...
  with SSHTunnelForwarder(
      ...
      try:
          with db.cursor() as cur:
              ...
              endD = Datestart[0:4]
              # print('select * from trade.sales where product_name = “活动物” LIMIT 3')
              arr = {}
              cur.execute('select import_country, sum(total_sales) as sale from trade.sales where export_country="%s" and product_name = "%s" and transaction_year >= "%s" and transaction_year <= "%s" group by import_country order by sale desc' %(exportName, ss, startD, endD))
              result = cur.fetchall()
              length = len(result)
              for i in range(length):
                if result[i][1] != 0:
                  keyword = "result"+str(i)
                  arr.update({keyword: result[i]})
              # terminal = {**terminal,**arr}
      finally:
          db.close()
  chartname = []
  chartvalue = []
  ...
  plot(fig, validate=False, filename='./templates/fig2.html',
         auto_open=False)
  simplename = []
  simplevalue = []
  together = 0.0
  ...
  constract_two = together * 0.02
  for i in range(len(result)):
    if result[i][1] > constract_two:
  ...
  plot(fig, validate=False, filename='./templates/simpleBarChart.html',
         auto_open=False)
  fig = px.bar(x=chartname, y=chartvalue)
  plot(fig, validate=False, filename='./templates/fig3.html',
         auto_open=False)
  # fig = px.choropleth(locations=["AUS", "BRA", "POL", "KOR"], locationmode="country names", color=[1,2,3,4], scope="world")
  chartcode = []
  for i in chartname:
    if i=="丹麦":
  ...
  plot(fig, validate=False, filename='./templates/fig6.html',
         auto_open=False)
  return render(request, 'showalldata.html', {'arr':arr.items()})

def mapcolor(request):
  return render(request, 'mapshow.html')

def mapdata(request):
  global terminal 
  return render(request, 'fig5.html', terminal)
